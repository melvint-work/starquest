...
# Sets up all scenario files and pre-builds the naive image for comparison.
...
# ── app.py (shared across both builds) ────────────────────────────────────────
...
# ── 01-naive-build/Dockerfile (pre-built for size comparison) ─────────────────
...
# Wait for the naive build to finish before signalling ready
wait
